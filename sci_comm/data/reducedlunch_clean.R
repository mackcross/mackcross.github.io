...
my_shapefile <- readOGR(dsn = "~/Documents/mackcross/sci_comm/shapefiles/original_Iowa/IowaCounties.shp")
...
grouped <- merged_df %>%
  ...
  summarize(ratio = sum(k12_enrollment, na.rm = TRUE) / sum(total_reduced_free, na.rm = TRUE),
            .groups = "drop")
head(grouped)

write.csv(grouped, "Documents/research/primate_map/project_materials/sci_comm/data/cleaned_grouped_data.csv", row.names = FALSE)
